						...
						if ( $key == 'outcome' ) :
							...
							foreach ( $value as $outcome ) :
								...
								if ( array_key_exists( $team_id, $totals ) && is_array( $totals[ $team_id ] ) && array_key_exists( $outcome, $totals[ $team_id ] ) ) :
									...
									$totals[ $team_id ]['eventminutes'] = (int) $totals[ $team_id ]['eventminutes'] + (int) $minutes;
									$totals[ $team_id ][ $outcome ] ++;

									// Add to home or away stats
									if ( 0 === $i ) :
										$totals[ $team_id ]['eventsplayed_home'] ++;
										$totals[ $team_id ]['eventminutes_home'] = (int) $totals[ $team_id ]['eventminutes_home'] + (int) $minutes;
										$totals[ $team_id ][ $outcome . '_home' ] ++;
									else :
										$totals[ $team_id ]['eventsplayed_away'] ++;
										$totals[ $team_id ]['eventminutes_away'] = (int) $totals[ $team_id ]['eventminutes_away'] + (int) $minutes;
									...
									if ( sp_is_home_venue( $team_id, $event->ID ) ) :
										...
										$totals[ $team_id ]['eventminutes_venue'] = (int) $totals[ $team_id ]['eventminutes_venue'] + (int) $minutes;
